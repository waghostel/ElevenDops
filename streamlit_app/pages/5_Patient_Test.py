...
if st.session_state.selected_agent_id:
    ...
    async def fetch_audio_files(knowledge_ids):
        """Fetch audio files for all knowledge IDs concurrently."""
    ...
    try:
        # Use stored agent from session state (no re-fetch needed!)
        current_agent = st.session_state.selected_agent
        
        if current_agent and current_agent.knowledge_ids:
            audio_files = run_async(fetch_audio_files(current_agent.knowledge_ids))
