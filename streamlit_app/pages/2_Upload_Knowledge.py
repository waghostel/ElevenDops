...
with st.container(border=True):
    ...
    try:
        ...
        if not documents:
            ...
            for doc in documents:
                # Check if we are editing this document
                if st.session_state.get("editing_doc_id") == doc.knowledge_id:
                     with st.container(border=True):
                        ...
                        with btn_c2:
                            if st.button("Cancel", key=f"cancel_edit_{doc.knowledge_id}"):
                                st.session_state.editing_doc_id = None
                                st.rerun()
                    ...
                    with col3:
                        if st.button("Edit", key=f"edit_btn_{doc.knowledge_id}"):
                            st.session_state.editing_doc_id = doc.knowledge_id
                            st.rerun()
