...
function Stop-ServerJobs {
    ...
    if (Test-Path ".server_jobs.json") {
        try {
            $jobInfo = Get-Content ".server_jobs.json" | ConvertFrom-Json
            ...
            Remove-Item ".server_jobs.json" -Force -ErrorAction SilentlyContinue
...
Write-Host "   To start servers: .\start_server.ps1" -ForegroundColor Cyan
